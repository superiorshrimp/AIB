...
def F_goal(W,C,no_area,flag):
    if flag == 0:
        sum_denominator = 0
        matrix_for_all_solution[no_area] = {0}
        for i in range(12):
            sum_denominator = sum_denominator + W[i][0]

        for i in range(12):
            new_vector = K_m(W,C,i,no_area,0);
            for j in range(no_area):
                matrix_for_all_solution[j] = matrix_for_all_solution[j] + new_vector[j]
        ...
        for i in range(12):
            matrix_for_all_solution = matrix_for_all_solution +K_m(W,C,i,no_area,1)
